...
// console(indekseGoreFenomen(fenomenler,0));
...
function fenomenEkle(arr, sıraNo, profileName, number_OfFollowers, numberOfPost, platformName) {

  let fenomen = {
    "number": sıraNo,
    "profile": profileName,
    "followers": number_OfFollowers,
    "posts": numberOfPost,
    "platform": platformName
  }

  arr[arr.length] = fenomen;
  return arr;
}
...
function enFenomenler(arr) {
  ...
  for(let i of arr){
    if(i.followers >100000000){
      result.push(i.profile);
    }
  }
...
function fenomenGonderimSayisi(arr,profileName){
  ...
  return "You gave unexisting profileName";
}
...
function platformaGoreCokGonderiYapanFenomen(arr, platformName){
  ...
  for(let i = 0; i < arr.length; i++){
    if (arr[i].platform === platformName && arr[i].posts > max){
      max = arr[i].posts;
    }
  }
