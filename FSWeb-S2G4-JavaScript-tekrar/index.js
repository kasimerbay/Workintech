...
let mix = sorted(sayilar); // just not to call sorted() 2 times
...
sayilar.forEach( element => {if (element % 3 === 0) ucetambolunenler.push(element);});

// console.log(ucetambolunenler);

	//3c çözümü:
	
ucebolunenlerintoplami = ucetambolunenler.reduce( (init, current) => init + current);
...
function update(sayilar) {
	let count = {};
	  
	for (let i = 0; i < sayilar.length; i++) {
		count[sayilar[i]] = count[sayilar[i]] ? count[sayilar[i]] += 1 : 1;
	}
	  
	let tekraredensayilar = [];
	  
	for (let i = 0; i < Object.keys(count).length; i++) {
		tekraredensayilar.push(`${Object.entries(count)[i][0]} sayısı ${Object.entries(count)[i][1]} kere tekrar edilmiştir`);
	}
	return tekraredensayilar;
}
	
tekraredensayilar = update(sayilar);
