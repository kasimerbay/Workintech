...
function cumlelereDonustur(arr, seperator = "," ){
	return arr.map( (array) => array.join(seperator));
}
...
function paragrafOlustur(arr, callback1, callback2){
	let result = callback2(cumleler, " ");

	return callback1(result[1], result[3], result[5], result[7], result[9]);
}
...
/* 	GÖREV 4:
		Yeni kurulmuş bir mesajlaşma startup firması atılan mesajları emojilerle zenginleştirmek istiyor. Bunun için emojiler adında bir nesne tanımlamışlar. 
			Kullanıcının gönderdiği mesaj stringi içinde emojiler nesnesinin anahtarı(index) bulunuyorsa, bu işareti otomatik olarak anahtara ait değerde 
			tanımlanmış emoji ile değiştirecek bir fonksiyon geliştirmek istiyorlar. ÖRNEK: Mesaj içinde ":)" sembolü bulunursa mesajı alan kişi bu sembolü "🙂" 
			olarak görecek. Burdan yola çıkarak emojileriDonustur fonksiyonuna aşağıdakileri uygulayın.
...
function emojileriDonustur(str, obj){
	...
	str.split(" ").map( (word) => {
		if(Object.keys(obj).includes(word.toLowerCase())){
		  str = str.replace(word, obj[word.toLowerCase()]);
		}
	  });
