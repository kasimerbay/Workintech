...
function dizi25Cesitmi(arr){
  ...
  // Dizi içerisinde tekrarlı tat olup olmadığını kontrol etmek için
  // döngü oluşturup result içerisinde yoksa o tat result içerisine
  // push edildi.
  ...
  // Döngü bittikten sonra result ile arr boyu karşılaştırıp verilen array için çeşit sayısı
  // ile result'ın uzunluğu aynı mı değil mi onu dönüyor.
...
function cesitEkle(arr, new_taste){
  /* My way (amele method)

    let result = [];

    result.push(new_taste);
    for (let i = 0; i < arr.length; i++){
      result.push[arr[i]];
    }

  */
  // return [new_taste, ...arr]; -> This a method for adding elements to an array.
  return [new_taste].concat(arr); //-> Fastest way of adding an element to an array in a given index
}
...
function ismeGoreCesitCikar(arr, taste){

  let index = arr.indexOf(taste); // verilen taste 'in indexini alma

  arr.splice(index,1); // splice metodu ile sadece verilen indexten itibaren ikinci parametre kadar olan elemanları attık.
...
function ismeGoreFiltrele(arr, taste){
  // Her bir tat için verilen taste'in string içinde include edilip edilmediğine bakıyor.
  let result = [];
...
function sumOfArrays (arr1, arr2) { // Verilen İki
...
// console.log(rastgeleTatlar (orijinalTatlar, yeniTatlar, bolgeselTatlar, mevsimlikTatlar));
